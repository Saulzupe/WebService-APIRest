package org.szunigap.webapp.jaxws.controllers;
...
import org.szunigap.webapp.jaxws.services.CursoService;
...
@RequestScoped
@Path("/cursos")
@Produces(MediaType.APPLICATION_JSON) // Lo que devuelve la respuesta
public class CursoRestController {
    @Inject
    private CursoService service;
    @GET
    public List<Curso> listar() {  // public List<Curso> listar()
        return service.listar(); //return service.listar
    }
    // Por id
    @GET
    @Path("/{id}")
    ...
    // Crear metodo post
    @POST
    @Consumes(MediaType.APPLICATION_JSON) //Formato en el que se envia, consume el parametro en el request
    ...
    public Response editar(@PathParam("id") Long id, Curso curso) {
        // Buscamos el curso por el id, los datos de persistencia que existen los modificamos con los datos del curso que estamos consumiendo
        Optional<Curso> cursoOptional = service.porId(id);
        if (cursoOptional.isPresent()){
